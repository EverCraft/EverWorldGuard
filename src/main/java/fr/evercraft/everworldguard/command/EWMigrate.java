...
public class EWMigrate extends ESubCommand<EverWorldGuard > {
	
	public EWMigrate(final EverWorldGuard plugin, final EWCommand command) {
        super(plugin, command, "migrate");
    }

	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EWPermissions.MIGRATE.get());
	}
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> args) throws CommandException {
		if (args.size() == 1) {
			...
			// Error : SQL disable
				...
				EWMessages.MIGRATE_DISABLE.sendTo(source);
			}
	...
	public Text getButtonConfirmationConf(){
		return EWMessages.MIGRATE_CONF_CONFIRMATION_VALID.getText().toBuilder()
					.onHover(TextActions.showText(EWMessages.MIGRATE_CONF_CONFIRMATION_VALID_HOVER.getText()))
